...
transformed data{
  ...
  real        mse0            = dot_self(y0 - X0 * hist_mle);
}
parameters {
  vector[K] beta;
...
model {
  ...
  real sigmasq_shape_a0 = sigmasq_shape + 0.5 * (a0*N0 - K);
  real sigmasq_scale_a0  = sigmasq_scale  + 0.5 * (a0 * (N0 - K) * mse0);
  ...
  // real sigmasq_scale_a0 = sigmasq_scale +
  ...
  // sigmasq    ~ inv_gamma(sigmasq_shape_a0, sigmasq_scale_a0);
  ...
  target += sigmasq_shape_a0 * log(sigmasq_scale_a0) - 0.5 * K * log(a0) -
    lgamma(sigmasq_shape_a0);
}
