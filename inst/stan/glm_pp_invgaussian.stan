...
data {
  ...
  vector<lower=0>[nobs] y0;            // historical response vector
...
model {
  vector[nobs] eta;
  vector[nobs] mu;
  real lambda = inv(dispersion);                   // shape param for inverse gaussian likelihood
  ...
  if ( a0 > 0 ) {
    ...
    mu      = linkinv(eta, link);
    target += a0 * inversegaussian_lpdf(y0 | mu, lambda );
  }
}
